...
import com.huawei.excelsior.jet.assembler.cbc.CbcFileFormat.{BytecodeReference, FieldReference, FieldReferenceWithRefType, MethodReference, RawData, Signature, StringLiteral}

trait SymbolAdapter {
  def adapt(symbol: Symbol): BytecodeReference

  final def field(symbol: Symbol): FieldReferenceWithRefType = adapt(symbol).asInstanceOf[FieldReferenceWithRefType]
