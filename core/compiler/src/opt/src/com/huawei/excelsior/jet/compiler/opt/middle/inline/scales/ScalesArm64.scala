...
import com.huawei.excelsior.jet.assembler.arm64.immediates.{FloatImm, ShiftedImm12}
import com.huawei.excelsior.common.CodeHelpers.shouldNotReachHere
import com.huawei.excelsior.jet.assembler.AsmType.*
import com.huawei.excelsior.jet.compiler.opt.ir.Universe

trait ScalesArm64 extends Scales { self: Universe =>

  override def nodeWeight(node: Node): Double = ScalesArm64Impl.nodeWeight(node)

  private [scales] object ScalesArm64Impl extends ScalesImpl {
    val instrSize = 4
    ...
    override def checkedOpIntConstWeight(v: Long) = {
      if (ShiftedImm12.canEncode(v)) instrSize // TODO tune for arm64
      else instrSize * 2
    }

    override def checkedOpIntDivRemWeight(n: Node) = n match {
      case CheckedOp(CheckedOp.Kind.DIV, _, _) => instrSize
      case _ => shouldNotReachHere(n)
    }

    override def checkedOpIntHighMulWeight(n: Node) = 0 // TODO: update after real implementation
    override def checkedOpIntLogicWeight = instrSize
    override def checkedOpIntAddSubWeight = checkedOpIntLogicWeight
    override def checkedOpIntMulWeight = checkedOpIntLogicWeight
    ...
    override def nodeWeight(n: Node): Double = n match {
      ...
      case _: AnyNull => 0
      case IConst(ic) =>
        if (ShiftedImm12.canEncode(ic)) instrSize // TODO tune for arm64
        else instrSize * 2
      case _: LConst => 0 // TODO: update after real implementation
      case FConst(fc) => if (FloatImm.canEncode(fc, Width.W32)) instrSize else instrSize * 3 // TODO tune for arm64
      case DConst(dc) => if (FloatImm.canEncode(dc, Width.W64)) instrSize else instrSize * 6 // TODO tune for arm64

      case _: ImportedIndex   => instrSize
      case _: ConstString     => ldrLiteralWeight + instrSize

      case _: GCPoint => instrSize * 2
